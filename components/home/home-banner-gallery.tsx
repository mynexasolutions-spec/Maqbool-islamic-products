...
export function HomeBannerGallery({ banners, marketSlug }: { banners: HomeBannerRecord[]; marketSlug: MarketSlug }) {
  ...
  return (
    <section aria-labelledby="home-banner-heading" className="site-container py-[70px]">
      <div className="mb-7 flex items-end justify-between gap-4">
        ...
        <Link href={marketHref(marketSlug, "/shop")} className="min-h-11 rounded-md px-3 py-3 text-sm font-bold text-forest underline-offset-4 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold">Shop all</Link>
      </div>
      <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {banners.map((banner, index) => {
          ...
          return <li key={banner.id} className={index === 0 && banners.length % 2 === 1 ? "sm:col-span-2 lg:col-span-1" : ""}>{banner.link_url ? <Link href={marketHref(marketSlug, banner.link_url)} className="group block overflow-hidden rounded-xl border border-[#ebdcb9] bg-white shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold">{media}</Link> : <div className="group overflow-hidden rounded-xl border border-[#ebdcb9] bg-white shadow-sm">{media}</div>}</li>;
        })}
      </ul>
    </section>
  );
