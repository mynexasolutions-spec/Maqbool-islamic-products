      ...
      {!items.length ? (
        <div className="grid min-h-[55vh] place-items-center text-center">
          <div>
            ...
            <button type="button" onClick={() => { setOpen(false); router.push("/shop"); }} className="mt-6 min-h-11 rounded-md bg-forest px-6 text-sm font-semibold text-white">Explore the shop</button>
          </div>
        </div>
      ) : (
        <div className="flex min-h-[calc(100vh-110px)] flex-col">
              ...
              <li key={item.id} className="grid grid-cols-[76px_1fr] gap-4 border-b border-[#eee7d8] pb-5">
                ...
                <div>
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    ...
                    <strong className="text-sm text-forest">{formatPrice(item.price * item.quantity)}</strong>
                  </div>
          ...
          <div className="sticky bottom-0 mt-6 border-t border-[#e4d8c1] bg-white pt-5">
            <div className="flex items-center justify-between"><span className="text-sm font-semibold text-muted">Subtotal</span><strong className="font-heading text-xl text-forest">{formatPrice(subtotal)}</strong></div>
            <p className="mt-2 text-xs text-muted">Shipping, offers, and COD charges are calculated at checkout.</p>
            <button type="button" onClick={() => { setOpen(false); router.push(customer ? "/checkout" : "/login?returnTo=%2Fcheckout"); }} className="mt-5 min-h-12 w-full rounded-md bg-forest px-5 text-sm font-bold text-white hover:bg-forest-light">
