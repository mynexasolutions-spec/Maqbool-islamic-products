...
export function CatalogCard({ product }: { product: Product }) {
  return (
    <article className="group relative overflow-hidden rounded-lg border border-[#eee8d8] bg-white transition duration-300 hover:-translate-y-1 hover:shadow-[0_18px_45px_rgba(15,56,44,.10)]">
      ...
      <Link href={`/shop/${product.slug}`} className="block rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold focus-visible:ring-offset-2">
        ...
        <div className="p-4">
          ...
          <div className="mt-3 flex items-baseline gap-2">
            ...
            {product.originalPrice ? <span className="text-xs text-muted line-through">{formatPrice(product.originalPrice)}</span> : null}
          </div>
        </div>
      </Link>
    </article>
